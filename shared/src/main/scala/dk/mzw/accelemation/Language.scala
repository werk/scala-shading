...
object Language {
    ...
    implicit class Vector3WithOperations(v : Term[(Double, Double, Double)]){
        ...
        def xx : Vec3 = Term(Field("xx", v.untyped))
        def xy : Vec3 = Term(Field("xy", v.untyped))
        def xz : Vec3 = Term(Field("xz", v.untyped))
        def yx : Vec3 = Term(Field("yx", v.untyped))
        def yy : Vec3 = Term(Field("yy", v.untyped))
        def yz : Vec3 = Term(Field("yz", v.untyped))
        def zx : Vec3 = Term(Field("zx", v.untyped))
        def zy : Vec3 = Term(Field("zy", v.untyped))
        def zz : Vec3 = Term(Field("zz", v.untyped))
